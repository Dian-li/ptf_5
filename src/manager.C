...
int TManager::startHttpServer() {
  ...
  if (unlikely(0 != evhttp_bind_socket(m_httpd, TPTFConfig::getInstance()->getListenAddr(), TPTFConfig::getInstance()->getManagerPort()))) {
    ...
    if (0 == strncmp("/ptf/new_test", url, 18)) {  //传输参数
      ...
      if (req_body[0] != '{') {
        ...
        if (reader.parse(string(req_body), value)) {
          kvmap["tps"] = value["tps"].asString();
          kvmap["duration"] = value["duration"].asString();
          kvmap["scriptname"] = value["scriptname"].asString();
          kvmap["protocol"] = value["protocol"].asString();
          kvmap["file"] = value["file"].asString();
        }
        for (auto &item : kvmap) {
        ...
        ptf_dir += "/task/";
        string filename = kvmap["scriptname"];
        string protocol = kvmap["protocol"];
