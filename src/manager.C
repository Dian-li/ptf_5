...
int TManager::startHttpServer() {
  ...
  if (unlikely(
      0 != evhttp_bind_socket(m_httpd,
                              TPTFConfig::getInstance()->getListenAddr(),
    ...
    if (0 == strncmp("/ptf/new_test", url, 18)) {  //传输参数
      ...
      if (req_body[0] != '{') {
        ...
        if (protocol == "TCP") {  //tcp
          ...
          createSocketPool();
          for (int i = 0; i < m_threadPool.size();
               ++i) {  //init connect pool,the number is the same as the number of thread pool
            string connStr = pScript->getConnStr();
            std::cout << connStr << std::endl;
            CSocket *cSocket = NULL;
            if (csocket_map.size() > 0) {
              cSocket =
                  csocket_map[connStr]->getSocket();  //got the socket connect,the number of connections is the same as the number of threads
            }
            if (cSocket != NULL) {
